...
function loadProductsFromCanonical() {
  ...
  for (const product of canonical.products) {
    ...
    const commerceProduct = {
      ...
      status: product.meta.status === 'enabled' ? 1 : 2,
      visibility: mapCanonicalVisibilityToCommerce(product.meta.visibility),
      tax_class_name: mapCanonicalTaxClass(product.meta.taxClass),
    ...
    for (const [key, value] of Object.entries(product.attributes)) {
      if (Array.isArray(value)) {
        commerceProduct[key] = value.join(',');
      } else {
...
/**
 * Map canonical tax class to Commerce tax class name
 */
function mapCanonicalTaxClass(taxClass) {
  const map = {
    'taxable_goods': 'Taxable Goods',
    'none': 'None'
  };
  return map[taxClass] || 'Taxable Goods';
}
...
/**
 * Map canonical image file to Commerce image path
 */
function mapCanonicalImagePath(imageFile) {
  if (!imageFile) return '';
  
  // Commerce format: /s/t/str-49c283de.jpg (first two chars as path)
  const sku = imageFile.replace('.jpg', '');
  return `/${sku.slice(0,1).toLowerCase()}/${sku.slice(1,2).toLowerCase()}/${imageFile}`;
}
